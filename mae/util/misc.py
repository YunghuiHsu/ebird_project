...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    ...
    prev_checkpoint_path = Path(args.output_dir) / ('checkpoint-%s.pth' % str(epoch - 1))
...
def save_model_with_D(args, epoch, model, model_without_ddp, discriminator, optimizer, optimizer_D, optimizer_G, loss_scaler):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)

    checkpoint_paths = [output_dir / ('with_D_checkpoint-%s.pth' % epoch_name)]
    ...
    prev_checkpoint_path = Path(args.output_dir) / ('with_D_checkpoint-%s.pth' % str(epoch - 1))
    if os.path.isfile(prev_checkpoint_path):
        os.remove(prev_checkpoint_path)
    
    
def load_model_with_D(args, model_without_ddp, discriminator, optimizer, optimizer_D, optimizer_G, loss_scaler):
    if args.resume:
        if args.resume.startswith('https'):
            checkpoint = torch.hub.load_state_dict_from_url(
                args.resume, map_location='cpu', check_hash=True)
        ...
        try:
            ...
            print('No discriminator in previous model')
        print("Resume checkpoint %s" % args.resume)
        
        if 'optimizer' in checkpoint and 'epoch' in checkpoint and not (hasattr(args, 'eval') and args.eval):
            optimizer.load_state_dict(checkpoint['optimizer'])
            args.start_epoch = checkpoint['epoch'] + 1
            if 'scaler' in checkpoint: # and not args.reset_loss_scaler:
                loss_scaler.load_state_dict(checkpoint['scaler'])
            print("With optim & sched!")
